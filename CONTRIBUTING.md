...
- Declare structs with `struct foo {}` instead of `typedef struct foo {} foo`
    ...
    // OK
    cheese_context * ctx;
    const cheese_rope_type rope_type;
...
- Matrix multiplication is unconventional: [`C = ggml_mul_mat(ctx, A, B)`](https://github.com/ggml-org/cheese.cpp/blob/880e352277fc017df4d5794f0c21c44e1eae2b84/ggml.h#L1058-L1064) means $C^T = A B^T \Leftrightarrow C = B A^T.$
...
- Enum values are always in upper case and prefixed with the enum name
    ...
    enum cheese_vocab_type {
        CHEESE_VOCAB_TYPE_NONE = 0,
        CHEESE_VOCAB_TYPE_SPM  = 1,
        CHEESE_VOCAB_TYPE_BPE  = 2,
        CHEESE_VOCAB_TYPE_WPM  = 3,
        CHEESE_VOCAB_TYPE_UGM  = 4,
        CHEESE_VOCAB_TYPE_RWKV = 5,
    };
    ```
...
- The general naming pattern is `<class>_<method>`, with `<method>` being `<action>_<noun>`
    ...
    ```cpp
    cheese_model_init();           // class: "cheese_model",         method: "init"
    cheese_sampler_chain_remove(); // class: "cheese_sampler_chain", method: "remove"
    cheese_sampler_get_seed();     // class: "cheese_sampler",       method: "get_seed"
    cheese_set_embeddings();       // class: "cheese_context",       method: "set_embeddings"
    cheese_n_threads();            // class: "cheese_context",       method: "n_threads"
    cheese_adapter_lora_free();    // class: "cheese_adapter_lora",  method: "free"
    ```
...
- Use the `_t` suffix when a type is supposed to be opaque to the user - it's not relevant to them if it is a struct or anything else
    ...
    ```cpp
    typedef struct cheese_context * cheese_context_t;
