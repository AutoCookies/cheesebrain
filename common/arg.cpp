...
#define CHEESE_MAX_URL_LENGTH 2084 // Maximum URL Length in Chrome: 2083
...
static std::initializer_list<enum cheese_example> mmproj_examples = {
    CHEESE_EXAMPLE_MTMD,
    CHEESE_EXAMPLE_SERVER,
    CHEESE_EXAMPLE_CLI,
};
...
static const std::vector<common_arg> & get_common_arg_defs() {
    static const std::vector<common_arg> options = [] {
        ...
        auto ctx = common_params_parser_init(params, CHEESE_EXAMPLE_SERVER, nullptr);
        return ctx.options;
    }();
    return options;
}

common_arg & common_arg::set_examples(std::initializer_list<enum cheese_example> examples) {
    this->examples = examples;
    return *this;
}

common_arg & common_arg::set_excludes(std::initializer_list<enum cheese_example> excludes) {
...
bool common_arg::in_example(enum cheese_example ex) {
    return examples.find(ex) != examples.end();
}

bool common_arg::is_exclude(enum cheese_example ex) {
...
bool common_arg::get_value_from_env(std::string & output) const {
    ...
    if (!args_neg.empty()) {
        // for compatibility, we need to check CHEESE_ARG_NO_ env as well
        std::string neg_env = env;
        string_replace_all(neg_env, "CHEESE_ARG_", "CHEESE_ARG_NO_");
...
std::vector<std::string> common_arg::get_env() const {
    ...
    if (!args_neg.empty() && env) {
        // for compatibility, we need to add CHEESE_ARG_NO_ variant
        std::string neg_env = env;
        string_replace_all(neg_env, "CHEESE_ARG_", "CHEESE_ARG_NO_");
...
static void parse_tensor_buffer_overrides(const std::string & value, std::vector<cheese_model_tensor_buft_override> & overrides) {
...
static bool common_params_handle_remote_preset(common_params & params, cheese_example ex) {
...
static bool common_params_parse_ex(int argc, char ** argv, common_params_context & ctx_arg) {
    ...
    // pad tensor_buft_overrides for cheese_params_fit:
    const size_t ntbo = cheese_max_tensor_buft_overrides();
...
static void common_params_print_usage(common_params_context & ctx_arg) {
    ...
    for (auto & opt : ctx_arg.options) {
        // in case multiple CHEESE_EXAMPLE_* are set, we prioritize the CHEESE_EXAMPLE_* matching current example
        if (opt.is_sparam) {
    ...
    // TODO: maybe convert enum cheese_example to string
...
static void common_params_print_completion(common_params_context & ctx_arg) {
    ...
    std::set<std::string> executables = {
        "cheese-batched",
        "cheese-batched-bench",
        "cheese-bench",
        "cheese-cli",
        "cheese-completion",
        "cheese-convert-cheese2c-to-ggml",
        "cheese-cvector-generator",
        "cheese-embedding",
        "cheese-eval-callback",
        "cheese-export-lora",
        "cheese-gen-docs",
        "cheese-gguf",
        "cheese-gguf-hash",
        ...
        "cheese-infill",
        "cheese-mtmd-cli",
        "cheese-llava-clip-quantize-cli",
        "cheese-lookahead",
        "cheese-lookup",
        "cheese-lookup-create",
        "cheese-lookup-merge",
        "cheese-lookup-stats",
        "cheese-parallel",
        "cheese-passkey",
        ...
        "cheese-qwen2vl-cli",
        "cheese-retrieval",
        "cheese-save-load-state",
        "cheese-server",
        "cheese-simple",
        "cheese-simple-chat",
        "cheese-speculative",
        "cheese-speculative-simple",
        "cheese-tokenize",
        "cheese-tts",
        "cheese-vdot"
    };
    ...
    for (const auto& exe : executables) {
        printf("complete -F _cheese_completions %s\n", exe.c_str());
    }
}
...
bool common_params_to_map(int argc, char ** argv, cheese_example ex, std::map<common_arg, std::string> & out_map) {
...
bool common_params_parse(int argc, char ** argv, common_params & params, cheese_example ex, void(*print_usage)(int, char **)) {
...
static std::string list_builtin_chat_templates() {
    ...
    int32_t res = cheese_chat_builtin_templates(nullptr, 0);
    supported_tmpl.resize(res);
    res = cheese_chat_builtin_templates(supported_tmpl.data(), supported_tmpl.size());
    std::ostringstream msg;
...
common_params_context common_params_parser_init(common_params & params, cheese_example ex, void(*print_usage)(int, char **)) {
    // per-example default params
    // we define here to make sure it's included in cheese-gen-docs
    if (ex == CHEESE_EXAMPLE_COMPLETION) {
        params.use_jinja = false;   // disable jinja by default

    } else if (ex == CHEESE_EXAMPLE_MTMD) {
        params.use_jinja = false;   // disable jinja by default
        params.sampling.temp = 0.2; // lower temp by default for better quality

    } else if (ex == CHEESE_EXAMPLE_SERVER) {
     ...
     * rules:
     * - all examples inherit options from CHEESE_EXAMPLE_COMMON
     * - if CHEESE_EXAMPLE_* is set (other than COMMON), we only show the option in the corresponding example
     * - if both {CHEESE_EXAMPLE_COMMON, CHEESE_EXAMPLE_*,} are set, we will prioritize the CHEESE_EXAMPLE_* matching current example
     */
    auto add_opt = [&](common_arg arg) {
        if ((arg.in_example(ex) || arg.in_example(CHEESE_EXAMPLE_COMMON)) && !arg.is_exclude(ex)) {
    ...
    add_opt(common_arg(
        ...
        [](common_params &) {
            fprintf(stderr, "version: %d (%s)\n", CHEESE_BUILD_NUMBER, CHEESE_COMMIT);
            fprintf(stderr, "built with %s for %s\n", CHEESE_COMPILER, CHEESE_BUILD_TARGET);
            exit(0);
        }
    ));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        {"--completion-bash"},
        "print source-able bash completion script for cheese.cpp",
        [](common_params & params) {
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_LOOKUP}));
    add_opt(common_arg(
        {"-t", "--threads"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_LOOKUP, CHEESE_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, int value) {
            params.n_ctx = value;
            if (value == 0) {
                // disable context reduction in cheese_params_fit if the user explicitly requests the full context size:
                params.fit_params_min_ctx = UINT32_MAX;
            }
        }
    ).set_env("CHEESE_ARG_CTX_SIZE"));
    add_opt(common_arg(
        {"-n", "--predict", "--n-predict"}, "N",
        string_format(
            ex == CHEESE_EXAMPLE_COMPLETION
    ...
    add_opt(common_arg(
        ...
        string_format("use full-size SWA cache (default: %s)\n"
            "[(more info)](https://github.com/ggml-org/cheese.cpp/pull/13194#issuecomment-2868343055)", params.swa_full ? "true" : "false"),
        [](common_params & params) {
            params.swa_full = true;
        }
    ).set_env("CHEESE_ARG_SWA_FULL"));
    add_opt(common_arg(
        {"--ctx-checkpoints", "--swa-checkpoints"}, "N",
        string_format("max number of context checkpoints to create per slot (default: %d)"
            "[(more info)](https://github.com/ggml-org/cheese.cpp/pull/15293)", params.n_ctx_checkpoints),
        [](common_params & params, int value) {
            params.n_ctx_checkpoints = value;
        }
    ).set_env("CHEESE_ARG_CTX_CHECKPOINTS").set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-cram", "--cache-ram"}, "N",
        string_format("set the maximum cache size in MiB (default: %d, -1 - no limit, 0 - disable)"
            "[(more info)](https://github.com/ggml-org/cheese.cpp/pull/16391)", params.cache_ram_mib),
        [](common_params & params, int value) {
            params.cache_ram_mib = value;
        }
    ).set_env("CHEESE_ARG_CACHE_RAM").set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-kvu", "--kv-unified"},
    ...
    ).set_env("CHEESE_ARG_KV_UNIFIED").set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_PERPLEXITY, CHEESE_EXAMPLE_BATCHED, CHEESE_EXAMPLE_BENCH, CHEESE_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--context-shift"},
        {"--no-context-shift"},
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_IMATRIX, CHEESE_EXAMPLE_PERPLEXITY}).set_env("CHEESE_ARG_CONTEXT_SHIFT"));
    add_opt(common_arg(
        {"--chunks"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_IMATRIX, CHEESE_EXAMPLE_PERPLEXITY, CHEESE_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg({ "-fa", "--flash-attn" }, "[on|off|auto]",
                       string_format("set Flash Attention use ('on', 'off', or 'auto', default: '%s')",
                                     cheese_flash_attn_type_name(params.flash_attn_type)),
                       [](common_params & params, const std::string & value) {
                           if (is_truthy(value)) {
                               params.flash_attn_type = CHEESE_FLASH_ATTN_TYPE_ENABLED;
                           } else if (is_falsey(value)) {
                               params.flash_attn_type = CHEESE_FLASH_ATTN_TYPE_DISABLED;
                           } else if (is_autoy(value)) {
                               params.flash_attn_type = CHEESE_FLASH_ATTN_TYPE_AUTO;
                           } else {
                               throw std::runtime_error(
                                   string_format("error: unknown value for --flash-attn: '%s'\n", value.c_str()));
                           }
                       }).set_env("CHEESE_ARG_FLASH_ATTN"));
    add_opt(common_arg(
        {"-p", "--prompt"}, "PROMPT",
    ...
    ).set_excludes({CHEESE_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_DIFFUSION, CHEESE_EXAMPLE_MTMD}));
    add_opt(common_arg(
        {"--perf"},
        {"--no-perf"},
        string_format("whether to enable internal libcheese performance timings (default: %s)", params.no_perf ? "true" : "false"),
        [](common_params & params, bool value) {
            params.no_perf = !value;
            params.sampling.no_perf = !value;
        }
    ).set_env("CHEESE_ARG_PERF"));
    add_opt(common_arg(
        {"--show-timings"},
        {"--no-show-timings"},
    ...
    ).set_examples({CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_SHOW_TIMINGS"));
    add_opt(common_arg(
        {"-f", "--file"}, "FNAME",
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_DIFFUSION}));
    add_opt(common_arg(
        {"--in-file"}, "FNAME",
    ...
    ).set_examples({CHEESE_EXAMPLE_IMATRIX}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION}));
    add_opt(common_arg(
        {"--prompt-cache"}, "FNAME",
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"-sp", "--special"},
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_MTMD, CHEESE_EXAMPLE_EMBEDDING, CHEESE_EXAMPLE_RETRIEVAL, CHEESE_EXAMPLE_PERPLEXITY, CHEESE_EXAMPLE_DEBUG}));
    add_opt(common_arg(
        {"--spm-infill"},
        string_format(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}));
    add_opt(common_arg(
        {"--samplers"}, "SAMPLERS",
    ...
    add_opt(common_arg(
        {"-s", "--seed"}, "SEED",
        string_format("RNG seed (default: %d, use random seed for %d)", params.sampling.seed, CHEESE_DEFAULT_SEED),
        [](common_params & params, const std::string & value) {
    ...
    ).set_sparam().set_env("CHEESE_ARG_TOP_K"));
    add_opt(common_arg(
        {"--top-p"}, "N",
    ...
    add_opt(common_arg(
        ...
        string_format("adaptive-p: select tokens near this probability (valid range 0.0 "
                      ...
                      "[(more info)](https://github.com/ggml-org/cheese.cpp/pull/17927)",
    ...
    ).set_sparam().set_env("CHEESE_ARG_BACKEND_SAMPLING"));
    add_opt(common_arg(
        {"--pooling"}, "{none,mean,cls,last,rank}",
        "pooling type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.pooling_type = CHEESE_POOLING_TYPE_NONE; }
            else if (value == "mean") { params.pooling_type = CHEESE_POOLING_TYPE_MEAN; }
            else if (value == "cls")  { params.pooling_type = CHEESE_POOLING_TYPE_CLS;  }
            else if (value == "last") { params.pooling_type = CHEESE_POOLING_TYPE_LAST; }
            else if (value == "rank") { params.pooling_type = CHEESE_POOLING_TYPE_RANK; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_examples({CHEESE_EXAMPLE_EMBEDDING, CHEESE_EXAMPLE_RETRIEVAL, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_DEBUG}).set_env("CHEESE_ARG_POOLING"));
    add_opt(common_arg(
        {"--attention"}, "{causal,non-causal}",
        "attention type for embeddings, use model default if unspecified",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "causal") { params.attention_type = CHEESE_ATTENTION_TYPE_CAUSAL; }
            else if (value == "non-causal") { params.attention_type = CHEESE_ATTENTION_TYPE_NON_CAUSAL; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_examples({CHEESE_EXAMPLE_EMBEDDING}));
    add_opt(common_arg(
        {"--rope-scaling"}, "{none,linear,yarn}",
        "RoPE frequency scaling method, defaults to linear unless specified by the model",
        [](common_params & params, const std::string & value) {
            /**/ if (value == "none") { params.rope_scaling_type = CHEESE_ROPE_SCALING_TYPE_NONE; }
            else if (value == "linear") { params.rope_scaling_type = CHEESE_ROPE_SCALING_TYPE_LINEAR; }
            else if (value == "yarn") { params.rope_scaling_type = CHEESE_ROPE_SCALING_TYPE_YARN; }
            else { throw std::invalid_argument("invalid value"); }
        }
    ).set_env("CHEESE_ARG_ROPE_SCALING_TYPE"));
    add_opt(common_arg(
        {"--rope-scale"}, "N",
    ...
    ).set_env("CHEESE_ARG_ROPE_FREQ_BASE"));
    add_opt(common_arg(
        {"--rope-freq-scale"}, "N",
    ...
    ).set_env("CHEESE_ARG_ROPE_FREQ_SCALE"));
    add_opt(common_arg(
        {"--yarn-orig-ctx"}, "N",
    ...
    ).set_env("CHEESE_ARG_YARN_ORIG_CTX"));
    add_opt(common_arg(
        {"--yarn-ext-factor"}, "N",
    ...
    ).set_env("CHEESE_ARG_YARN_EXT_FACTOR"));
    add_opt(common_arg(
        {"--yarn-attn-factor"}, "N",
    ...
    ).set_env("CHEESE_ARG_YARN_ATTN_FACTOR"));
    add_opt(common_arg(
        {"--yarn-beta-slow"}, "N",
    ...
    ).set_env("CHEESE_ARG_YARN_BETA_SLOW"));
    add_opt(common_arg(
        {"--yarn-beta-fast"}, "N",
    ...
    ).set_env("CHEESE_ARG_YARN_BETA_FAST"));
    add_opt(common_arg(
        {"-gan", "--grp-attn-n"}, "N",
    ...
    ).set_env("CHEESE_ARG_GRP_ATTN_N").set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_PASSKEY}));
    add_opt(common_arg(
        {"-gaw", "--grp-attn-w"}, "N",
    ...
    ).set_env("CHEESE_ARG_GRP_ATTN_W").set_examples({CHEESE_EXAMPLE_COMPLETION}));
    add_opt(common_arg(
        {"-kvo", "--kv-offload"},
        {"-nkvo", "--no-kv-offload"},
    ...
    ).set_env("CHEESE_ARG_CACHE_TYPE_K"));
    add_opt(common_arg(
    ...
    ).set_env("CHEESE_ARG_CACHE_TYPE_V"));
    add_opt(common_arg(
        {"--hellaswag"},
    ...
    ).set_examples({CHEESE_EXAMPLE_PERPLEXITY}));
    add_opt(common_arg(
        {"--hellaswag-tasks"}, "N",
    ...
    ).set_env("CHEESE_ARG_DEFRAG_THOLD"));
    if (ex == CHEESE_EXAMPLE_SERVER) {
        ...
        ).set_env("CHEESE_ARG_N_PARALLEL").set_examples({CHEESE_EXAMPLE_SERVER}));
    } else {
        add_opt(common_arg(
            {"-np", "--parallel"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"-cb", "--cont-batching"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_CONT_BATCHING"));
    add_opt(common_arg(
        {"-mm", "--mmproj"}, "FILE",
    ...
    ).set_examples(mmproj_examples).set_env("CHEESE_ARG_MMPROJ"));
    add_opt(common_arg(
        {"-mmu", "--mmproj-url"}, "URL",
    ...
    ).set_examples(mmproj_examples).set_env("CHEESE_ARG_MMPROJ_URL"));
    add_opt(common_arg(
        {"--mmproj-auto"},
    ...
    ).set_examples(mmproj_examples).set_env("CHEESE_ARG_MMPROJ_AUTO"));
    add_opt(common_arg(
        {"--mmproj-offload"},
        {"--no-mmproj-offload"},
    ...
    ).set_examples(mmproj_examples).set_env("CHEESE_ARG_MMPROJ_OFFLOAD"));
    add_opt(common_arg(
        {"--image", "--audio"}, "FILE",
    ...
    ).set_examples({CHEESE_EXAMPLE_MTMD, CHEESE_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"--image-min-tokens"}, "N",
    ...
    ).set_examples(mmproj_examples).set_env("CHEESE_ARG_IMAGE_MIN_TOKENS"));
    add_opt(common_arg(
        {"--image-max-tokens"}, "N",
    ...
    ).set_examples(mmproj_examples).set_env("CHEESE_ARG_IMAGE_MAX_TOKENS"));
    if (cheese_supports_rpc()) {
        add_opt(common_arg(
            {"--rpc"}, "SERVERS",
    ...
    add_opt(common_arg(
        ...
        "see https://github.com/ggml-org/cheese.cpp/issues/1437",
        [](common_params & params, const std::string & value) {
    ...
    ).set_env("CHEESE_ARG_OVERRIDE_TENSOR"));
    add_opt(common_arg(
        {"-otd", "--override-tensor-draft"}, "<tensor name pattern>=<buffer type>,...",
        "override tensor buffer type for draft model", [](common_params & params, const std::string & value) {
            parse_tensor_buffer_overrides(value, params.speculative.tensor_buft_overrides);
        }
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}));
    add_opt(common_arg(
        {"-cmoe", "--cpu-moe"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_CPU_MOE_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_N_CPU_MOE_DRAFT"));
    ...
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (!cheese_supports_gpu_offload()) {
                ...
                fprintf(stderr, "warning: one possible reason is that cheese.cpp was compiled without GPU support\n");
                fprintf(stderr, "warning: consult docs/build.md for compilation instructions\n");
            }
        }
    ).set_env("CHEESE_ARG_N_GPU_LAYERS"));
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (arg_next == "none") {
                params.split_mode = CHEESE_SPLIT_MODE_NONE;
            } else if (arg_next == "layer") {
                params.split_mode = CHEESE_SPLIT_MODE_LAYER;
            } else if (arg_next == "row") {
                params.split_mode = CHEESE_SPLIT_MODE_ROW;
            } else {
                throw std::invalid_argument("invalid value");
            }
            if (!cheese_supports_gpu_offload()) {
                fprintf(stderr, "warning: cheese.cpp was compiled without support for GPU offload. Setting the split mode has no effect.\n");
            }
        }
    ).set_env("CHEESE_ARG_SPLIT_MODE"));
    add_opt(common_arg(
        ...
        [](common_params & params, const std::string & value) {
            ...
            if (split_arg.size() >= cheese_max_devices()) {
                throw std::invalid_argument(
                    string_format("got %zu input configs, but system only has %zu devices", split_arg.size(), cheese_max_devices())
                );
            }
            for (size_t i = 0; i < cheese_max_devices(); ++i) {
                if (i < split_arg.size()) {
            ...
            if (!cheese_supports_gpu_offload()) {
                fprintf(stderr, "warning: cheese.cpp was compiled without support for GPU offload. Setting a tensor split has no effect.\n");
            }
        }
    ).set_env("CHEESE_ARG_TENSOR_SPLIT"));
    add_opt(common_arg(
        {"-mg", "--main-gpu"}, "INDEX",
        ...
        [](common_params & params, int value) {
            ...
            if (!cheese_supports_gpu_offload()) {
                fprintf(stderr, "warning: cheese.cpp was compiled without support for GPU offload. Setting the main GPU has no effect.\n");
            }
        }
    ).set_env("CHEESE_ARG_MAIN_GPU"));
    add_opt(common_arg(
        { "-fit", "--fit" }, "[on|off]",
    ...
    ).set_examples({CHEESE_EXAMPLE_COMMON, CHEESE_EXAMPLE_EXPORT_LORA}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_ALIAS"));
    add_opt(common_arg(
        {"--tags"}, "STRING",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_TAGS"));
    add_opt(common_arg(
        {"-m", "--model"}, "FNAME",
        ex == CHEESE_EXAMPLE_EXPORT_LORA
            ? "model path from which to load base model"
            : "model path to load",
        [](common_params & params, const std::string & value) {
            params.model.path = value;
        }
    ).set_examples({CHEESE_EXAMPLE_COMMON, CHEESE_EXAMPLE_EXPORT_LORA}).set_env("CHEESE_ARG_MODEL"));
    add_opt(common_arg(
    ...
    ).set_env("CHEESE_ARG_DOCKER_REPO"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_RETRIEVAL}));
    add_opt(common_arg(
        {"--chunk-size"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_PASSKEY, CHEESE_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"--pos"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_PASSKEY}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_IMATRIX, CHEESE_EXAMPLE_CVECTOR_GENERATOR, CHEESE_EXAMPLE_EXPORT_LORA, CHEESE_EXAMPLE_TTS, CHEESE_EXAMPLE_FINETUNE}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_BENCH, CHEESE_EXAMPLE_PARALLEL}));
    add_opt(common_arg(
        {"-tgs"},
    ...
    ).set_examples({CHEESE_EXAMPLE_BENCH}));
    add_opt(common_arg(
        {"-ntg"}, "n0,n1,...",
    ...
    ).set_examples({CHEESE_EXAMPLE_EMBEDDING, CHEESE_EXAMPLE_DEBUG}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_HOST"));
    add_opt(common_arg(
        {"--port"}, "PORT",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_PORT"));
    add_opt(common_arg(
        {"--path"}, "PATH",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_STATIC_PATH"));
    add_opt(common_arg(
        {"--api-prefix"}, "PREFIX",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_API_PREFIX"));
    add_opt(common_arg(
        {"--webui-config"}, "JSON",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_WEBUI_CONFIG"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_WEBUI_CONFIG_FILE"));
    add_opt(common_arg(
        {"--webui"},
        {"--no-webui"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_WEBUI"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_DEBUG}).set_env("CHEESE_ARG_EMBEDDINGS"));
    add_opt(common_arg(
        {"--rerank", "--reranking"},
        string_format("enable reranking endpoint on server (default: %s)", "disabled"),
        [](common_params & params) {
            params.embedding = true;
            params.pooling_type = CHEESE_POOLING_TYPE_RANK;
        }
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_API_KEY"));
    add_opt(common_arg(
        {"--api-key-file"}, "FNAME",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_SSL_KEY_FILE"));
    add_opt(common_arg(
        {"--ssl-cert-file"}, "FNAME",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_SSL_CERT_FILE"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_CHAT_TEMPLATE_KWARGS"));
    add_opt(common_arg(
        {"-to", "--timeout"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_TIMEOUT"));
    add_opt(common_arg(
        {"--threads-http"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_THREADS_HTTP"));
    add_opt(common_arg(
        {"--cache-prompt"},
        {"--no-cache-prompt"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_CACHE_PROMPT"));
    add_opt(common_arg(
        {"--cache-reuse"}, "N",
        string_format(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_CACHE_REUSE"));
    add_opt(common_arg(
        {"--metrics"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_ENDPOINT_METRICS"));
    add_opt(common_arg(
        {"--props"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_ENDPOINT_PROPS"));
    add_opt(common_arg(
        {"--slots"},
        {"--no-slots"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_ENDPOINT_SLOTS"));
    add_opt(common_arg(
        {"--slot-save-path"}, "PATH",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_MODELS_DIR"));
    add_opt(common_arg(
        {"--models-preset"}, "PATH",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_MODELS_PRESET"));
    add_opt(common_arg(
        {"--models-max"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_MODELS_MAX"));
    add_opt(common_arg(
        {"--models-autoload"},
        {"--no-models-autoload"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_MODELS_AUTOLOAD"));
    add_opt(common_arg(
        {"--jinja"},
        {"--no-jinja"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_MTMD}).set_env("CHEESE_ARG_JINJA"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_THINK"));
    add_opt(common_arg(
        {"--reasoning-budget"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_THINK_BUDGET"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_MTMD}).set_env("CHEESE_ARG_CHAT_TEMPLATE"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_COMPLETION, CHEESE_EXAMPLE_CLI, CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_CHAT_TEMPLATE_FILE"));
    add_opt(common_arg(
        {"--prefill-assistant"},
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER}).set_env("CHEESE_ARG_PREFILL_ASSISTANT"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_CVECTOR_GENERATOR}));
    add_opt(common_arg(
        {"--negative-file"}, "FNAME",
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_LOOKUP, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_DRAFT_MAX"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_LOOKUP, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_DRAFT_MIN"));
    add_opt(common_arg(
        {"--draft-p-split"}, "P",
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE}).set_env("CHEESE_ARG_DRAFT_P_SPLIT"));
    add_opt(common_arg(
        {"--draft-p-min"}, "P",
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_DRAFT_P_MIN"));
    add_opt(common_arg(
        {"-cd", "--ctx-size-draft"}, "N",
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_CTX_SIZE_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_N_GPU_LAYERS_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_SPECULATIVE, CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}).set_env("CHEESE_ARG_MODEL_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_env("CHEESE_ARG_CACHE_TYPE_K_DRAFT"));
    add_opt(common_arg(
    ...
    ).set_env("CHEESE_ARG_CACHE_TYPE_V_DRAFT"));
    ...
    ).set_examples({CHEESE_EXAMPLE_TTS, CHEESE_EXAMPLE_SERVER}));
     add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_TTS}));

    add_opt(common_arg(
        {"--diffusion-steps"}, "N",
        string_format("number of diffusion steps (default: %d)", params.diffusion.steps),
        [](common_params & params, int value) { params.diffusion.steps = value; }
    ).set_examples({ CHEESE_EXAMPLE_DIFFUSION }));
    add_opt(common_arg(
        {"--diffusion-visual"},
    ...
    ).set_examples({ CHEESE_EXAMPLE_FINETUNE }));
    ...
    ).set_examples({CHEESE_EXAMPLE_DEBUG}));
    add_opt(common_arg(
    ...
    ).set_examples({CHEESE_EXAMPLE_EMBEDDING, CHEESE_EXAMPLE_SERVER}));
    ...
    ).set_examples({CHEESE_EXAMPLE_SERVER, CHEESE_EXAMPLE_CLI}));
