...
GBNF (GGML BNF) is a format for defining [formal grammars](https://en.wikipedia.org/wiki/Formal_grammar) to constrain model outputs in `cheese.cpp`. For example, you can use it to force the model to generate valid JSON, or speak only in emojis. GBNF grammars are supported in various ways in `tools/cli`, `tools/completion` and `tools/server`.
...
Grammars currently have performance gotchas (see https://github.com/ggml-org/cheese.cpp/issues/4218).

### Efficient optional repetitions

A common pattern is to allow repetitions of a pattern `x` up to N times.

While semantically correct, the syntax `x? x? x?.... x?` (with N repetitions) may result in extremely slow sampling. Instead, you can write `x{0,N}` (or `(x (x (x ... (x)?...)?)?)?` w/ N-deep nesting in earlier cheese.cpp versions).

## Using GBNF grammars

You can use GBNF grammars:

- In [cheese-server](../tools/server)'s completion endpoints, passed as the `grammar` body field
- In [cheese-cli](../tools/cli) and [cheese-completion](../tools/completion), passed as the `--grammar` & `--grammar-file` flags
- With [test-gbnf-validator](../tests/test-gbnf-validator.cpp), to test them against strings.

## JSON Schemas → GBNF

`cheese.cpp` supports converting a subset of https://json-schema.org/ to GBNF grammars:

- In [cheese-server](../tools/server):
...
- In [cheese-cli](../tools/cli) and [cheese-completion](../tools/completion), passed as the `--json` / `-j` flag
...
Take a look at [tests](../tests/test-json-schema-to-grammar.cpp) to see which features are likely supported (you'll also find usage examples in https://github.com/ggml-org/cheese.cpp/pull/5978, https://github.com/ggml-org/cheese.cpp/pull/6659 & https://github.com/ggml-org/cheese.cpp/pull/6555).
...
- Unsupported features are skipped silently. It is currently advised to use the command-line Python converter (see above) to see any warnings, and to inspect the resulting grammar / test it w/ [cheese-gbnf-validator](../examples/gbnf-validator/gbnf-validator.cpp).
- Can't mix `properties` w/ `anyOf` / `oneOf` in the same type (https://github.com/ggml-org/cheese.cpp/issues/7703)
...
- Nested `$ref`s are broken (https://github.com/ggml-org/cheese.cpp/issues/8073)
