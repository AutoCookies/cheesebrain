 ...
 * It is a playground for trying multimodal support in cheese.cpp.
...
struct mtmd_cli_context {
    ...
    cheese_tokens antiprompt_tokens;
    ...
    mtmd_cli_context(common_params & params) : cheese_init(common_init_from_params(params)) {
        model = cheese_init->model();
        lctx = cheese_init->context();
        vocab = cheese_model_get_vocab(model);
        smpl = common_sampler_init(model, params.sampling);
        n_threads = params.cpuparams.n_threads;
        batch = cheese_batch_init(1, 0, 1); // batch for next token generation
        ...
        if (!cheese_model_chat_template(model, nullptr) && params.chat_template.empty()) {
    ...
    bool check_antiprompt(const cheese_tokens & generated_tokens) {
...
static int generate_response(mtmd_cli_context & ctx, int n_predict) {
    ...
    for (int i = 0; i < n_predict; i++) {
        ...
        cheese_token token_id = common_sampler_sample(ctx.smpl, ctx.lctx, -1);
        generated_tokens.push_back(token_id);
        common_sampler_accept(ctx.smpl, token_id, true);

        if (cheese_vocab_is_eog(ctx.vocab, token_id) || ctx.check_antiprompt(generated_tokens)) {
            LOG("\n");
        ...
        if (cheese_decode(ctx.lctx, ctx.batch)) {
...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_MTMD, show_additional_info)) {
        return 1;
    }
    ...
    LOG_WRN("      For normal use cases, please use the standard cheese-cli\n");
    ...
    if (is_single_turn) {
        ...
        if (params.prompt.find(mtmd_default_marker()) == std::string::npos) {
            for (size_t i = 0; i < params.image.size(); i++) {
                ...
                // ref: https://github.com/ggml-org/cheese.cpp/pull/17616
        ...
        while (!g_is_interrupted) {
            ...
            if (line == "/clear") {
                ...
                cheese_memory_clear(cheese_get_memory(ctx.lctx), true);
                if (eval_system_prompt_if_present()) {
                    return 1;
                }
    ...
    LOG("\n\n");
    cheese_perf_context_print(ctx.lctx);
