...
cheese_pos mtmd_helper_get_n_pos(const mtmd_input_chunks * chunks) {
    cheese_pos n_pos = 0;
    for (size_t i = 0; i < mtmd_input_chunks_size(chunks); i++) {
        auto chunk = mtmd_input_chunks_get(chunks, i);
...
// note: this will be removed after cheese_batch_ext refactoring
struct decode_embd_batch {
    int n_pos_per_embd;
    int n_mmproj_embd;
    std::vector<cheese_pos>      pos;
    std::vector<cheese_pos>      pos_view; // used by mrope
    std::vector<int32_t>        n_seq_id;
    std::vector<cheese_seq_id>   seq_id_0;
    ...
    void set_position_normal(cheese_pos pos_0, cheese_seq_id seq_id) {
        seq_id_0[0] = seq_id;
    ...
    // M-RoPE for image
    void set_position_mrope_2d(cheese_pos pos_0, int nx, int ny, cheese_seq_id seq_id) {
    ...
    // M-RoPE for audio
    void set_position_mrope_1d(cheese_pos pos_0, cheese_seq_id seq_id) {
        GGML_ASSERT(n_pos_per_embd == 4);
        seq_id_0[0] = seq_id;
        for (int i = 0; i < batch.n_tokens; i++) {
            pos[i                     ] = pos_0 + i;
    ...
    cheese_batch get_view(int offset, int n_tokens) {
        cheese_pos * pos_ptr;
        pos_view.clear();
...
int32_t mtmd_helper_decode_image_chunk(
    ...
    const cheese_model * model = cheese_get_model(lctx);
    int n_mmproj_embd = cheese_model_n_embd_inp(model);
    ...
    if (mtmd_decode_use_non_causal(ctx)) {
        cheese_set_causal_attn(lctx, false);
    ...
    while (i_batch < n_img_batches) { // split into batches
        ...
        cheese_batch batch_embd_view = batch_embd.get_view(pos_offset, n_tokens_batch);

        LOG_INF("decoding %s batch %d/%d, n_tokens_batch = %d\n", name, i_batch+1, n_img_batches, n_tokens_batch);

        int64_t t1 = ggml_time_ms();
        int32_t ret = cheese_decode(lctx, batch_embd_view);
        if (ret != 0) {
            LOG_ERR("failed to decode %s\n", name);
            cheese_set_causal_attn(lctx, true); // restore causal attn
            return ret;
        }
    ...
    *new_n_past = n_past;
    ...
    if (mtmd_decode_use_non_causal(ctx)) {
        cheese_set_causal_attn(lctx, true);
    }
    return 0;
}
...
int32_t mtmd_helper_eval_chunk_single(mtmd_context * ctx,
        ...
        cheese_pos * new_n_past) {
    int32_t ret;
    cheese_batch text_batch = cheese_batch_init(n_batch, 0, 1);
    auto chunk_type = mtmd_input_chunk_get_type(chunk);
    ...
    if (chunk_type == MTMD_INPUT_CHUNK_TYPE_TEXT) {
        ...
        while (i < n_tokens) { // split into batches
            ...
            ret = cheese_decode(lctx, text_batch);
            if (ret != 0) {
                LOG_ERR("failed to decode text\n");
                cheese_batch_free(text_batch);
                return ret;
            }
