...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_CLI)) {
        return 1;
    }
    ...
    if (params.conversation_mode == COMMON_CONVERSATION_MODE_DISABLED) {
        ...
        console::error("please use cheese-completion instead\n");
    }
    ...
    while (true) {
        ...
        if (string_starts_with(buffer, "/exit")) {
            ...
            if (inf.fim_sep_token != CHEESE_TOKEN_NULL) {
                cur_msg += common_token_to_piece(ctx_cli.ctx_server.get_cheese_context(), inf.fim_sep_token, true);
                cur_msg += fname;
                cur_msg.push_back('\n');
            } else {
                cur_msg += "--- File: ";
