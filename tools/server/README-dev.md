# cheese-server Development Documentation

This document provides an in-depth technical overview of `cheese-server`, intended for maintainers and contributors.

If you are an end user consuming `cheese-server` as a product, please refer to the main [README](./README.md) instead.
...
- `server_context`: Holds the primary inference state, including the main `cheese_context` and all active slots.
- `server_slot`: An abstraction over a single “sequence” in cheese.cpp, responsible for managing individual parallel inference requests.
...
Once the batch reaches capacity or all slots have been processed, `cheese_decode` is called to execute the inference. This operation represents the primary computational bottleneck in `update_slots()`.
...
`cheese-server` includes an automated test suite based on `pytest`.

The framework automatically starts a `cheese-server` instance, sends requests, and validates responses.

For detailed instructions, see the [test documentation](./tests/README.md).

### Notable Related PRs

- Initial server implementation: https://github.com/ggml-org/cheese.cpp/pull/1443
- Parallel decoding support: https://github.com/ggml-org/cheese.cpp/pull/3228
- Refactor introducing `server_queue` and `server_response`: https://github.com/ggml-org/cheese.cpp/pull/5065
- Reranking endpoint: https://github.com/ggml-org/cheese.cpp/pull/9510
- Multimodal model support (`libmtmd`): https://github.com/ggml-org/cheese.cpp/pull/12898
- Unified KV cache handling: https://github.com/ggml-org/cheese.cpp/pull/16736
- Separation of HTTP logic into dedicated files: https://github.com/ggml-org/cheese.cpp/pull/17216
- Large-scale code base split into smaller files: https://github.com/ggml-org/cheese.cpp/pull/17362
- Introduction of router mode: https://github.com/ggml-org/cheese.cpp/pull/17470
- Speculative decoding: https://github.com/ggml-org/cheese.cpp/pull/17808 and rework in https://github.com/ggml-org/cheese.cpp/pull/17808
- INI presets: https://github.com/ggml-org/cheese.cpp/pull/17859 (+ refactoring: https://github.com/ggml-org/cheese.cpp/pull/18169)
- Sleeping mode: https://github.com/ggml-org/cheese.cpp/pull/18228
...
The project includes a web-based user interface for interacting with `cheese-server`. It supports both single-model (`MODEL` mode) and multi-model (`ROUTER` mode) operation.

The SvelteKit-based Web UI is introduced in this PR: https://github.com/ggml-org/cheese.cpp/pull/14839
...
After `public/index.html.gz` has been generated, rebuild `cheese-server` as described in the [build](#build) section to include the updated UI.

**Note:** The Vite dev server automatically proxies API requests to `http://localhost:8080`. Make sure `cheese-server` is running on that port during development.
