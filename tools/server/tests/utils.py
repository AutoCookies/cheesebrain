...
class ServerProcess:
    ...
    model_hf_file: str | None = "tinycheeses/stories260K.gguf"
    model_alias: str = "tinycheese-2"
    ...
    def start(self, timeout_seconds: int | None = DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        elif "CHEESE_SERVER_BIN_PATH" in os.environ:
            server_path = os.environ["CHEESE_SERVER_BIN_PATH"]
        elif os.name == "nt":
            server_path = "../../../build/bin/Release/cheese-server.exe"
        else:
            server_path = "../../../build/bin/cheese-server"
        server_args = [
            "--host",
            self.server_host,
            "--port",
        ...
        self.process = subprocess.Popen(
            ...
            stdout=sys.stdout,
            stderr=sys.stdout,
            env={**os.environ, "CHEESE_CACHE": "tmp"} if "CHEESE_CACHE" not in os.environ else None,
        )
        ...
        if stream:
            ...
            for chunk in self.make_stream_request(method, path, data, headers):
                if chunk['choices']:
                    ...
                    # and a `usage` field containing the usage statistics (n.b., cheese-server also returns `timings` in
                    # the last chunk)
...
class ServerPreset:
    ...
    @staticmethod
    def tinycheese_infill() -> ServerProcess:
        server = ServerProcess()
        server.offline = True # will be downloaded by load_all()
        server.model_hf_repo = "ggml-org/test-model-stories260K-infill"
        server.model_hf_file = None
        server.model_alias = "tinycheese-infill"
        server.n_ctx = 2048
        server.n_batch = 1024
        server.n_slots = 1
        server.n_predict = 64
