...
struct server_slot {
    ...
    //       see https://github.com/ggml-org/cheese.cpp/pull/18283#issuecomment-3710175837
    ...
    void prompt_save(server_prompt_cache & prompt_cache) const {
        ...
        const size_t cur_size = cheese_state_seq_get_size_ext(ctx, id, 0);
        ...
        cheese_state_seq_get_data_ext(ctx, cur->data.data(), cur_size, id, 0);
    }
    ...
    void prompt_clear(bool allow_processing) {
        ...
        cheese_memory_seq_rm(cheese_get_memory(ctx), id, -1, -1);
        prompt.tokens.clear();
    }
    ...
    cheese_token  sampled; // in speculative mode, this is the last accepted token
    cheese_tokens drafted;
    ...
    void reset() {
        ...
        cheese_set_sampler(ctx, id, nullptr);
    ...
    void init_sampler() const {
        ...
        for (int i = 0; i < (int) prompt.tokens.size(); i++) {
            const cheese_token id = prompt.tokens[i];
    ...
    bool can_split() const {
        ...
        return
            !task->need_embd() ||
            (cheese_get_memory(ctx) && cheese_pooling_type(ctx) == CHEESE_POOLING_TYPE_LAST);
    }
    ...
    void copy_state_to(server_slot & other) const {
        ...
        cheese_memory_seq_rm(cheese_get_memory(ctx), other.id,     -1, -1);
        cheese_memory_seq_cp(cheese_get_memory(ctx), id, other.id, -1, -1);
...
public:
    ...
    mtmd_context * mctx = nullptr;
    const cheese_vocab * vocab = nullptr;
...
private:
    ...
    common_init_result_ptr cheese_init;

    cheese_context * ctx = nullptr;

    cheese_batch batch {};

    cheese_model_ptr model_dft;
    ...
    // load the model and initialize cheese_context
    ...
    bool load_model(const common_params & params) {
        ...
        cheese_init = common_init_from_params(params_base);

        model = cheese_init->model();
        ctx   = cheese_init->context();
        ...
        vocab = cheese_model_get_vocab(model);

        n_ctx = cheese_n_ctx(ctx);

        add_bos_token = cheese_vocab_get_add_bos(vocab);
        ...
        if (params_base.speculative.has_dft()) {
            ...
            params_dft.n_ctx        = params_spec.n_ctx == 0 ? cheese_n_ctx_seq(ctx) : params_spec.n_ctx;
            params_dft.n_batch      = cheese_n_ctx_seq(ctx);
            ...
            auto mparams_dft = common_model_params_to_cheese(params_dft);

            model_dft.reset(cheese_model_load_from_file(params_dft.model.path.c_str(), mparams_dft));
            ...
            params_base.speculative.cparams_dft = common_context_params_to_cheese(params_dft);
        }
        ...
        if (!cheese_memory_can_shift(cheese_get_memory(ctx))) {
            if (params_base.ctx_shift) {
                params_base.ctx_shift = false;
        ...
        const int n_ctx_train = cheese_model_n_ctx_train(model);

        int n_ctx_slot = cheese_n_ctx_seq(ctx);
        ...
        {
            const char * CHEESE_SERVER_SLOTS_DEBUG = getenv("CHEESE_SERVER_SLOTS_DEBUG");
            slots_debug = CHEESE_SERVER_SLOTS_DEBUG ? atoi(CHEESE_SERVER_SLOTS_DEBUG) : 0;
        ...
        {
            const int32_t n_batch = cheese_n_batch(ctx);
            batch = cheese_batch_init(std::max(n_batch, params_base.n_parallel), 0, 1);
        }
    ...
    bool launch_slot_with_task(server_slot & slot, server_task && task) {
        ...
        if (lora_all_alora(slot.lora)) {
            ...
            const uint64_t      n_invocation_tokens = cheese_adapter_get_alora_n_invocation_tokens(lora);
            const cheese_token * invocation_tokens   = cheese_adapter_get_alora_invocation_tokens  (lora);
        ...
        if (task.need_sampling()) {
            ...
            if (backend_sampling) {
                cheese_set_sampler(ctx, slot.id, common_sampler_get(slot.smpl.get()));
            } else {
                cheese_set_sampler(ctx, slot.id, nullptr);
            }
    ...
    bool process_token(completion_token_output & result, server_slot & slot) {
        ...
        if (!incomplete) {
            ...
            } else if (slot.has_next_token && !cheese_vocab_is_eog(vocab, result.tok) ) {
        ...
        if (cheese_vocab_is_eog(vocab, result.tok)) {
    ...
    void populate_token_probs(const server_slot & slot, completion_token_output & result, bool post_sampling, bool special, int idx) const {
        ...
        if (post_sampling) {
            ...
            std::vector<cheese_token_data> cur = get_token_probabilities(ctx, idx);
    ...
    void send_final_response(server_slot & slot) {
        ...
        if (slot.task->params.sampling.n_probs > 0) {
            if (!slot.task->params.stream && slot.stop == STOP_TYPE_WORD) {
                const cheese_tokens stop_word_toks = common_tokenize(ctx, slot.stopping_word, false);
    ...
    void send_embedding(const server_slot & slot, const cheese_batch & batch) {
        ...
        const int n_embd_out = cheese_model_n_embd_out(model);
        ...
        for (int i = 0; i < batch.n_tokens; ++i) {
            ...
            if (cheese_pooling_type(slot.ctx) == CHEESE_POOLING_TYPE_NONE) {
                embd = cheese_get_embeddings_ith(ctx, i);
            } else {
                embd = cheese_get_embeddings_seq(ctx, batch.seq_id[i][0]);
            }
    ...
    void send_rerank(const server_slot & slot, const cheese_batch & batch) {
        ...
        for (int i = 0; i < batch.n_tokens; ++i) {
            ...
            const float * embd = cheese_get_embeddings_seq(ctx, batch.seq_id[i][0]);
            if (embd == NULL) {
                embd = cheese_get_embeddings_ith(ctx, i);
            }
            ...
            case SERVER_TASK_TYPE_SLOT_SAVE:
                {
                    ...
                    const cheese_tokens & tokens = slot->prompt.tokens.get_text_tokens();
                    const size_t nwrite = cheese_state_seq_save_file(ctx, filepath.c_str(), slot->id, tokens.data(), token_count);
            ...
            case SERVER_TASK_TYPE_SLOT_RESTORE:
                {
                    ...
                    size_t nread = cheese_state_seq_load_file(ctx, filepath.c_str(), slot->id, tokens.data(), tokens.size(), &token_count);
                    if (nread == 0) {
            ...
            case SERVER_TASK_TYPE_GET_LORA:
                {
                    ...
                    for (size_t i = 0; i < loras.size(); ++i) {
                        ...
                        const uint64_t n_alora_tokens = cheese_adapter_get_alora_n_invocation_tokens(lora.ptr);
                        cheese_tokens alora_invocation_tokens;
                        if (n_alora_tokens) {
                            const cheese_token * alora_tokens = cheese_adapter_get_alora_invocation_tokens(lora.ptr);
    ...
    void update_slots() {
        ...
        for (server_slot & slot : slots) {
            if (slot.state == SLOT_STATE_GENERATING && slot.prompt.n_tokens() + 1 >= slot.n_ctx) {
                ...
                cheese_memory_seq_rm (cheese_get_memory(ctx), slot.id, n_keep            , n_keep + n_discard);
                cheese_memory_seq_add(cheese_get_memory(ctx), slot.id, n_keep + n_discard, slot.prompt.n_tokens(), -n_discard);

                // add generated tokens to cache
                // ref: https://github.com/ggml-org/cheese.cpp/pull/16818#discussion_r2473269481
                {
                    GGML_ASSERT(!slot.prompt.tokens.has_mtmd);

                    cheese_tokens new_tokens = slot.prompt.tokens.get_text_tokens(); // copy
        ...
        auto accept_special_token = [&](server_slot & slot, cheese_token token) {
        ...
        for (auto & slot : slots) {
            ...
            // TODO: rework to have a single draft cheese_context shared across all slots [TAG_SERVER_SPEC_REWORK]
            ...
            if (n_draft_max > 0) {
                ...
                const cheese_tokens & cached_text_tokens = slot.prompt.tokens.get_text_tokens();

                const auto & params_spec = slot.task->params.speculative;

                cheese_tokens draft = common_speculative_draft(slot.spec, params_spec, cached_text_tokens, slot.sampled);
        ...
        int32_t n_batch  = cheese_n_batch(ctx);
        int32_t n_ubatch = cheese_n_ubatch(ctx);
            ...
            for (auto & slot : slots) {
                ...
                if (slot.state == SLOT_STATE_PROCESSING_PROMPT || slot.state == SLOT_STATE_STARTED) {
                    ...
                    if (slot.state == SLOT_STATE_STARTED) {
                        ...
                        if (slot.task->need_logits() && !cheese_get_memory(ctx)) {
                        ...
                        if (!slot.can_split()) {
                            ...
                            if (
                                ...
                                const bool can_cache_reuse =
                                    cheese_memory_can_shift(cheese_get_memory(ctx)) &&
                                ...
                                if (can_cache_reuse && n_cache_reuse > 0) {
                                    ...
                                    while (head_c < slot.prompt.tokens.size() &&
                                        ...
                                        if (n_match >= (size_t) n_cache_reuse) {
                                            ...
                                            cheese_memory_seq_rm (cheese_get_memory(ctx), slot.id, head_p, head_c);
                                            cheese_memory_seq_add(cheese_get_memory(ctx), slot.id, head_c, head_c + n_match, kv_shift);
                            ...
                            cheese_pos pos_next = slot.prompt.tokens.pos_next(n_past);

                            // note: when n_swa == 0, the model does not use SWA, which is equivalent to a window of 1
                            const auto n_swa = std::max(1, cheese_model_n_swa(model));

                            // the largest pos_min required for a checkpoint to be useful
                            const auto pos_min_thold = std::max(0, pos_next - n_swa);

                            if (n_past > 0 && n_past < slot.prompt.n_tokens()) {
                                const auto pos_min = cheese_memory_seq_pos_min(cheese_get_memory(ctx), slot.id);
                                if (pos_min == -1) {
                                    SLT_ERR(slot, "n_past = %d, slot.prompt.tokens.size() = %d, seq_id = %d, pos_min = %d\n", n_past, (int) slot.prompt.tokens.size(), slot.id, pos_min);
                                    GGML_ABORT("pos_min == -1, but n_past > 0 - should not happen: https://github.com/ggml-org/cheese.cpp/pull/13833#discussion_r2116181237");
                                }
                                ...
                                if (slots_debug) {
                                    ...
                                    for (int i = np0; i < np1; i++) {
...

                                        {
                                            ...
                                            const auto piece = token != CHEESE_TOKEN_NULL ? common_token_to_piece(ctx, token) : "[mtmd]";
                                            ss0 << piece;
                                ...
                                if (pos_min > pos_min_thold) {
                                    ...
                                    if (!do_reset) {
                                        ...
                                        const size_t n = cheese_state_seq_set_data_ext(ctx, it->data.data(), checkpoint_size, slot.id, CHEESE_STATE_SEQ_FLAGS_PARTIAL_ONLY);
                                    ...
                                    if (do_reset) {
                                        SLT_WRN(slot, "forcing full prompt re-processing due to lack of cache data (likely due to SWA or hybrid/recurrent memory, see %s)\n",
                                                "https://github.com/ggml-org/cheese.cpp/pull/13194#issuecomment-2868343055");
                                        pos_next = 0;
                                        n_past = 0;
                                    }
                                }
                    ...
                    const cheese_pos p0 = slot.prompt.tokens.pos_next();

                    SLT_INF(slot, "n_tokens = %d, memory_seq_rm [%d, end)\n", slot.prompt.n_tokens(), p0);

                    if (!cheese_memory_seq_rm(cheese_get_memory(ctx), slot.id, p0, -1)) {
                    ...
                    do_checkpoint = do_checkpoint && (
                            cheese_model_is_recurrent(model) ||
                            cheese_model_is_hybrid(model) ||
                            (cheese_model_n_swa(model) > 0 && !params_base.swa_full)
                            );
                    ...
                    if (slot.prompt.n_tokens() == n_input_tokens) {
                        ...
                        const auto pos_min = cheese_memory_seq_pos_min(cheese_get_memory(ctx), slot.id);
                        const auto pos_max = cheese_memory_seq_pos_max(cheese_get_memory(ctx), slot.id);
                        ...
                        // note: we create the checkpoint before calling cheese_decode(), so the current batch is not
                        ...
                        if (do_checkpoint) {
                            ...
                            const size_t checkpoint_size = cheese_state_seq_get_size_ext(ctx, slot.id, CHEESE_STATE_SEQ_FLAGS_PARTIAL_ONLY);
                            ...
                            cheese_state_seq_get_data_ext(ctx, cur.data.data(), checkpoint_size, slot.id, CHEESE_STATE_SEQ_FLAGS_PARTIAL_ONLY);
        ...
        if (slot_batched) {
            ...
            cheese_set_embeddings(ctx, slot_batched->task->need_embd());
        }
        ...
        for (int32_t i = 0; i < batch.n_tokens; i = i_next) {
            ...
            const int ret = cheese_decode(ctx, batch_view);
            ...
            if (ret != 0) {
                {
                    ...
                    if (ret < -1) {
                        // TODO: update slot state based on cheese_memory_seq_pos_min() and cheese_memory_seq_pos_max()
                        err = "Compute error.";
                    }
            ...
            for (auto & slot : slots) {
                ...
                cheese_token id = common_sampler_sample(slot.smpl.get(), ctx, tok_idx);

                slot.i_batch = -1;

                common_sampler_accept(slot.smpl.get(), id, true);

                // here we have synchronized the cheese_context (due to the sampling above), so we can do time measurement
            ...
            for (auto & slot : slots) {
                ...
                cheese_memory_seq_rm(cheese_get_memory(ctx), slot.id, slot.prompt.n_tokens(), -1);
...
cheese_context * server_context::get_cheese_context() const {
    return impl->ctx;
}
...
server_context_meta server_context::get_meta() const {
    auto bos_id = cheese_vocab_bos(impl->vocab);
    auto eos_id = cheese_vocab_eos(impl->vocab);
    auto bos_token_str = bos_id != CHEESE_TOKEN_NULL ? common_token_to_piece(impl->ctx, bos_id, true) : "";
    auto eos_token_str = eos_id != CHEESE_TOKEN_NULL ? common_token_to_piece(impl->ctx, eos_id, true) : "";
    ...
    return server_context_meta {
        ...
        /* pooling_type           */ cheese_pooling_type(impl->ctx),
        ...
        /* fim_pre_token          */ cheese_vocab_fim_pre(impl->vocab),
        /* fim_sub_token          */ cheese_vocab_fim_suf(impl->vocab),
        /* fim_mid_token          */ cheese_vocab_fim_mid(impl->vocab),
        /* fim_pad_token          */ cheese_vocab_fim_pad(impl->vocab),
        /* fim_rep_token          */ cheese_vocab_fim_rep(impl->vocab),
        /* fim_sep_token          */ cheese_vocab_fim_sep(impl->vocab),

        /* model_vocab_type       */ cheese_vocab_type(impl->vocab),
        /* model_vocab_n_tokens   */ cheese_vocab_n_tokens(impl->vocab),
        /* model_n_ctx_train      */ cheese_model_n_ctx_train(impl->model),
        /* model_n_embd_inp       */ cheese_model_n_embd(impl->model),
        /* model_n_params         */ cheese_model_n_params(impl->model),
        /* model_size             */ cheese_model_size(impl->model),
    };
}
...
std::unique_ptr<server_res_generator> server_routes::handle_completions_impl(
    ...
    if (!stream) {
        ...
        // ref: https://github.com/ggml-org/cheese.cpp/pull/16486#discussion_r2419657309
...
void server_routes::init_routes() {
    ...
    this->get_metrics = [this](const server_http_req & req) {
        ...
        json all_metrics_def = json {
            {"counter", {{
                    ...
                    {"name",  "n_decode_total"},
                    {"help",  "Total number of cheese_decode() calls"},
        ...
        for (const auto & el : all_metrics_def.items()) {
            ...
            for (const auto & metric_def : metrics_def) {
                ...
                prometheus << "# HELP cheesecpp:" << name << " " << help  << "\n"
                            << "# TYPE cheesecpp:" << name << " " << type  << "\n"
                            << "cheesecpp:"        << name << " " << value << "\n";
            }
        }
    ...
    this->get_api_show = [this](const server_http_req &) {
        ...
        json data = {
            {
                "model_info", {
                    { "cheese.context_length", meta->slot_n_ctx },
                }
            },
            {"modelfile", ""},
            {"parameters", ""},
    ...
    this->post_infill = [this](const server_http_req & req) {
        ...
        std::string err;
        if (cheese_vocab_fim_pre(ctx_server.vocab) == CHEESE_TOKEN_NULL) {
            err += "prefix token is missing. ";
        }
        if (cheese_vocab_fim_suf(ctx_server.vocab) == CHEESE_TOKEN_NULL) {
            err += "suffix token is missing. ";
        }
        if (cheese_vocab_fim_mid(ctx_server.vocab) == CHEESE_TOKEN_NULL) {
    ...
    this->post_anthropic_count_tokens = [this](const server_http_req & req) {
        ...
        cheese_tokens tokens = tokenize_mixed(ctx_server.vocab, prompt, true, true);
            ...
            {"models", {
                {
                    ...
                    {"modified_at", ""},
                    {"size", ""},
                    {"digest", ""}, // dummy value, cheese.cpp does not support managing model file's hash
                    {"type", "model"},
                    {"description", ""},
                    {"tags", {""}},
                    {"capabilities", meta->has_mtmd ? json({"completion","multimodal"}) : json({"completion"})},
    ...
    this->post_tokenize = [this](const server_http_req & req) {
        ...
        if (body.count("content") != 0) {
            ...
            cheese_tokens tokens = tokenize_mixed(ctx_server.vocab, body.at("content"), add_special, parse_special);
    ...
    this->post_detokenize = [this](const server_http_req & req) {
        ...
        if (body.count("tokens") != 0) {
            const cheese_tokens tokens = body.at("tokens");
    ...
    this->post_rerank = [this](const server_http_req & req) {
        auto res = create_response();
        if (!params.embedding || params.pooling_type != CHEESE_POOLING_TYPE_RANK) {
...
std::unique_ptr<server_res_generator> server_routes::handle_embeddings_impl(const server_http_req & req, task_response_type res_type) {
    ...
    if (res_type != TASK_RESPONSE_TYPE_NONE && meta->pooling_type == CHEESE_POOLING_TYPE_NONE) {
    ...
    if (body.count("embd_normalize") != 0) {
        ...
        if (meta->pooling_type == CHEESE_POOLING_TYPE_NONE) {
