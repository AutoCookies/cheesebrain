...
json task_params::format_logit_bias(const std::vector<cheese_logit_bias> & logit_bias) const {
        ...
        if (grammar_triggers != data.end()) {
            for (const auto & t : *grammar_triggers) {
                ...
                if (ct.value.type == COMMON_GRAMMAR_TRIGGER_TYPE_WORD) {
                    ...
                    if (ids.size() == 1) {
                        auto token = ids[0];
                        if (std::find(params.sampling.preserved_tokens.begin(), params.sampling.preserved_tokens.end(), (cheese_token) token) == params.sampling.preserved_tokens.end()) {
...

    {
        ...
        if (logit_bias != data.end() && logit_bias->is_array()) {
            ...
            for (const auto & el : *logit_bias) {
                ...
                if (el.is_array() && el.size() == 2) {
                    ...
                    if (el[0].is_number_integer()) {
                        cheese_token tok = el[0].get<cheese_token>();
            ...
            for (const auto & el : logit_bias->items()) {
                ...
                char *end;
                cheese_token tok = strtol(key.c_str(), &end, 10);
                if (*end == 0) {
                    if (tok >= 0 && tok < n_vocab) {
                        params.sampling.logit_bias.push_back({tok, bias});
                    }
...
bool server_prompt_cache::load(server_prompt & prompt, const server_tokens & tokens_new, cheese_context * ctx, int32_t id_slot) {
    ...
    if (it_best != states.end()) {
        ...
        const size_t n = cheese_state_seq_set_data_ext(ctx, it_best->data.data(), size, id_slot, 0);
        if (n != size) {
