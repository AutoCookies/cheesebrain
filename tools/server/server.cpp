...
int main(int argc, char ** argv) {
    ...
    if (!common_params_parse(argc, argv, params, CHEESE_EXAMPLE_SERVER)) {
        return 1;
    }

    // validate batch size for embeddings
    // embeddings require all tokens to be processed in a single ubatch
    // see https://github.com/ggml-org/cheese.cpp/issues/12836
    ...
    // struct that contains cheese context and inference
    ...
    ctx_http.get ("/api/tags",            ex_wrapper(routes.get_models)); // ocheese specific endpoint. public endpoint (no API key check)
    ctx_http.post("/completion",          ex_wrapper(routes.post_completions)); // legacy
    ctx_http.post("/completions",         ex_wrapper(routes.post_completions));
    ctx_http.post("/v1/completions",      ex_wrapper(routes.post_completions_oai));
    ctx_http.post("/chat/completions",    ex_wrapper(routes.post_chat_completions));
    ctx_http.post("/v1/chat/completions", ex_wrapper(routes.post_chat_completions));
    ctx_http.post("/api/chat",            ex_wrapper(routes.post_chat_completions)); // ocheese specific endpoint
    ...
    if (is_router_server) {
        ...
        const char * router_port = std::getenv("CHEESE_SERVER_ROUTER_PORT");
        ...
        auto * ll_ctx = ctx_server.get_cheese_context();
        if (ll_ctx != nullptr) {
            cheese_memory_breakdown_print(ll_ctx);
        }
    }
